...
export default function AoVivoRoute() {
  ...
  // Efeito visual de ruído de estática de TV antiga (chuvisco) super resiliente a falhas de codec
  useEffect(() => {
    ...
    const renderNoise = () => {
      ...
      try {
        ...
        // Desenhar uma mensagem clássica de "FORA DO AR" ou "BUSCANDO SINAL" estilizada em cima do ruído
        ctx.fillStyle = "rgba(0, 0, 0, 0.82)";
        ctx.fillRect(w / 2 - 140, h / 2 - 35, 280, 70);

        ctx.strokeStyle = "#fbbf24"; // Amarelo clássico vintage
        ctx.lineWidth = 2;
        ...
        ctx.fillText("VIRTUAL TV - SINAL ANALÓGICO", w / 2, h / 2 - 10);

        ctx.fillStyle = "#fbbf24";
        ctx.font = "9px monospace";
        ctx.fillText("BUSCANDO ESTAÇÃO... TENTE SINTONIZAR", w / 2, h / 2 + 15);
      } catch (err) {
        // Fallback preventivo caso dê erro de render no canvas
  ...
  const initPlayer = useCallback((streamUrl: string, fallbackStage: number | boolean = 0, seekOffset: number = 0) => {
    const stage = typeof fallbackStage === "boolean" ? (fallbackStage ? 1 : 0) : fallbackStage;

    // Resetar estática de TV antiga ao sintonizar novo sinal!
    ...
    // Destruir instância anterior de Hls, importante para liberar memória de vídeo corporativa
    if (hlsRef.current) {
      hlsRef.current.destroy();
      hlsRef.current = null;
    }

    // Gerenciador central de erros de vídeo com estágios de contingência lineares (sem loop infinito)
    const onVideoError = () => {
      setIsLoading(false);
      const nextStage = stage + 1;
      console.warn(`[Sinal de Vídeo] Falha de reprodução no estágio ${stage}. Próximo estágio planejado: ${nextStage}`);

      // Se for um link do Google Drive para tentar alternar para o modo iframe (altamente estável contra cotas)
      const driveIframeUrl = getDriveIframeUrl(streamUrl);
      if (driveIframeUrl && !useDriveIframe) {
        console.warn("[Sinal de Vídeo] Ativando o modo Iframe reserva para Google Drive...");
      ...
      if (nextStage === 1) {
        // Estágio 1: Recupera com a URL de contingência do canal cadastrado
        const channelFallback = currentChannel.fallbackUrl || "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4";
        if (channelFallback && channelFallback !== streamUrl) {
          console.warn(`[Sinal de Vídeo] Tentando URL de contingência do canal (Stage 1): ${channelFallback}`);
          initPlayer(channelFallback, 1, seekOffset);
        } else {
          console.warn("[Sinal de Vídeo] URL de fallback idêntica à de falha. Avançando diretamente para o Stage 2.");
          initPlayer("https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4", 2, seekOffset);
        }
      } else if (nextStage === 2) {
        // Estágio 2: Usar vídeo de teste padrão universal MP4
        const universalMp4 = "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4";
        if (streamUrl !== universalMp4) {
          console.warn(`[Sinal de Vídeo] Tentando reprodutor seguro universal MP4 (Stage 2): ${universalMp4}`);
          initPlayer(universalMp4, 2, seekOffset);
        } else {
          console.warn("[Sinal de Vídeo] Vídeo universal MP4 falhou. Avançando para o Stage 3.");
          initPlayer("https://upload.wikimedia.org/wikipedia/commons/f/f1/Sintel_movie_teaser_480p.webm", 3, seekOffset);
        }
      } else if (nextStage === 3) {
        // Estágio 3: Usar filme WebM livre (importante para navegadores headless/testes Puppeteer sem codecs proprietários)
        const universalWebM = "https://upload.wikimedia.org/wikipedia/commons/f/f1/Sintel_movie_teaser_480p.webm";
        if (streamUrl !== universalWebM) {
          console.warn(`[Sinal de Vídeo] Tentando vídeo WebM de codec aberto (Stage 3): ${universalWebM}`);
          initPlayer(universalWebM, 3, seekOffset);
        } else {
          console.error("[Sinal de Vídeo] Falha irrecuperável em todas as mídias. Forçando estática de TV...");
        ...
        // Limite excedido ou parado preventivamente para congelar loops
        console.error("[Sinal de Vídeo] Limite de fallback excedido. Forçando estática de TV...");
        setUseAnalogStatic(true);
        setHasError(false);
        setIsLoading(false);
        setIsPlaying(false);
    ...
    // Verificar previamente se o formato do player é suportado pelo navegador atual para mitigar erros de codec no terminal
    const isHlsUrl = streamUrl.endsWith(".m3u8") || streamUrl.includes("m3u8");
    const isWebmUrl = streamUrl.endsWith(".webm") || streamUrl.includes("webm");
    const isMp4Url = streamUrl.endsWith(".mp4") || streamUrl.includes("google.com") || (!isHlsUrl && !isWebmUrl);

    let isSupported = true;
    if (isHlsUrl) {
      const hasNativeHls = video.canPlayType && video.canPlayType("application/vnd.apple.mpegurl") !== "";
      const hasHlsJs = Hls.isSupported();
      if (!hasNativeHls && !hasHlsJs) {
        console.warn("[Compatibilidade] HLS não é suportado pelo navegador atual.");
        isSupported = false;
      }
    } else if (isWebmUrl) {
      if (video.canPlayType && video.canPlayType("video/webm") === "") {
        console.warn("[Compatibilidade] Codec WebM não é suportado pelo navegador atual.");
        isSupported = false;
      }
    } else if (isMp4Url) {
      if (video.canPlayType && video.canPlayType("video/mp4") === "") {
        console.warn("[Compatibilidade] Codec MP4 não é suportado pelo navegador atual.");
        isSupported = false;
      }
    }

    if (!isSupported) {
      console.warn(`[Compatibilidade] Pulando reprodução direta de ${streamUrl} por falta de codec compatível.`);
      setTimeout(() => {
        onVideoError();
      }, 0);
      return;
    ...
    // Configurar se é arquivo MP4 comum/Google Drive ou stream M3U8, WebM ou se já estamos forçando o reprodução crua de contingência
    const isMp4 = streamUrl.endsWith(".mp4") || streamUrl.endsWith(".webm") || streamUrl.includes("google.com") || stage > 0;
    ...
    if (isMp4) {
      ...
      video.play()
        ...
        .catch((err) => {
          ...
          const isAutoplayRestrict = errName === "NotAllowedError" || errMsg.includes("allowed") || errMsg.includes("autoplay") || errMsg.includes("user gesture");

          if (isAutoplayRestrict) {
            ...
            video.play()
              ...
              .catch((e2) => {
                console.warn("[Play Failed após mute]", e2);
                setIsPlaying(false);
                video.removeEventListener("error", onVideoError);
                onVideoError();
              });
          } else {
            console.warn("[Play Failed] Erro de mídia detectado pelo navegador:", err);
            setIsPlaying(false);
            video.removeEventListener("error", onVideoError);
            onVideoError();
          }
  ...
  }, [isMuted, volume, currentChannel.fallbackUrl, useDriveIframe]);
      ...
      <main className={`flex-1 flex flex-col lg:flex-row ${isSmartTvMode ? "p-0" : "p-2 sm:p-4"} gap-4 overflow-hidden`}>
        ...
        <div className={`flex-1 flex flex-col justify-center ${isSmartTvMode ? "h-screen w-screen p-0" : "gap-3 sm:gap-4"} h-full`}>
          ...
          >
            ...
            {useAnalogStatic ? (
              <div id="retro-static-noise-screen" className="absolute inset-0 z-10 w-full h-full bg-zinc-950 flex flex-col items-center justify-center">
                <canvas 
                  ref={staticCanvasRef} 
                  className="w-full h-full absolute inset-0 block object-cover opacity-90"
                />
              </div>
